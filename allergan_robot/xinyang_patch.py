...
# df = pd.read_csv('C:/Users/HS01/PycharmProjects/pylearning/learn5/data/BITFINEX_BTCUSD_20180124_1T.csv')
df = pd.read_excel('E:/Hammer_Studio/tmp-project/咨询项目/艾尔建/爬虫项目/data_warehouse/12-10/2019-12-10-22-20-10-11431925772512-新氧-北京的数据-后羿采集器.xlsx', encoding='utf-8')
# exit()

# =====导入数据
# df = pd.read_csv(
#     # 该参数为数据在电脑中的路径
#     filepath_or_buffer='C:/Users/HS01/PycharmProjects/pylearning/learn5/data/BITFINEX_BTCUSD_20180124_1T.csv',
#     # 该参数代表数据的分隔符，csv文件默认是逗号。其他常见的是'/t'
#     sep=',',
#     # 该参数代表跳过数据文件的的第1行不读入
#     skiprows=1,
...
print(df.loc[:, '标题'])  # 选取指定的某一行，读取的数据是Series类型
...
col_name.insert(1, '类别')
col_name.insert(3, '剂量')
col_name.insert(4, '品牌')
df = df.reindex(columns=col_name)
df['区域'] = '北京'


df['剂量'] = df['标题'].str.extract(r'(\d+.\d+)', expand=False)
# print(title)
print(df)
exit()
test['剂量'] = test.rating.apply(lambda x: eval(x.split('/')[0]))
# 提取分母
...
# print(df[() & (df['symbol'] == 'AIDUSD')])
print(df[df['标题'].str.contains('隆鼻')])
exit()
